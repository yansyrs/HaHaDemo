...
public class WaveView extends View implements ViewTreeObserver.OnPreDrawListener {
  ...
  private int mShadowDy = 2;
  private int mShadowColor = SHADOW_COLOR;

  public void setShadowRadius(float radius) {
    setShadow(radius, mShadowColor);
  }

  public void setShadow(float radius, int color) {
    mShadowRadius = radius > 0 ? radius : mShadowRadius;
    mShadowColor = color;
    updateShadow();
  }

  private void updateShadow() {
    mShadowPaint.setShadowLayer(mShadowRadius, 0f, 2.0f, mShadowColor);
  }
