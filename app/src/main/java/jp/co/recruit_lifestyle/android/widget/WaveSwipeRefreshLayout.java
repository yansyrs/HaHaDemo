...
public class WaveSwipeRefreshLayout extends ViewGroup
   ...
   * 主色调
   */
  private static final int MAIN_COLOR = Color.rgb(63, 82, 181);//Color.rgb(41, 129, 235);
  private int mMainColor = MAIN_COLOR;

  /**
   * 刷新图标颜色
   */
  private static final int REFRESH_COLOR = Color.WHITE;
  private int mRefreshColor = REFRESH_COLOR;
  ...
  public WaveSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs,
            R.styleable.WaveSwipRefresh, defStyleAttr, 0);
    try {
      mShadowColor = a.getColor(R.styleable.WaveSwipRefresh_shadowColor, SHADOW_COLOR);
      mShadowSize = a.getDimension(R.styleable.WaveSwipRefresh_shadowSize, SHADOW_SIZE);
      mMainColor = a.getColor(R.styleable.WaveSwipRefresh_mainColor, MAIN_COLOR);
      mRefreshColor = a.getColor(R.styleable.WaveSwipRefresh_refreshColor, REFRESH_COLOR);
    } finally {
      a.recycle();
    }
  }
  ...
  private void layoutWaveView() {
    ...
    //yan
    mWaveView.setShadow(mShadowSize, mShadowColor);
    setColorSchemeColors(Color.WHITE, Color.WHITE);
    setWaveColor(mMainColor);
  }
